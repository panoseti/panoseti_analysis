...
def test_ray_vs_cli_meta_parity(
    ray_local_mode,
    dummy_l1_store: Path,
    dummy_model_path: Path,
    tmp_path: Path,
) -> None:
    """Ray and CPU paths must produce identical L2 lineage meta (dp, module, run_id)."""
    out_cli = tmp_path / "meta_cli_out"
    out_cli.mkdir()
    l2_store_cli = out_cli / "obs_TEST.cloud.module_1.zarr"

    out_ray = tmp_path / "meta_ray_out"
    out_ray.mkdir()
    ...
    assert record_ray.dp == "img16", f"Ray record dp={record_ray.dp!r}, expected 'img16'"
    assert record_ray.module == "1", f"Ray record module={record_ray.module!r}, expected '1'"
