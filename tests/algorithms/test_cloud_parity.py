...
def test_hann_window_equivalence(rng_image: np.ndarray) -> None:
    ...
    np.testing.assert_allclose(
        scipy_win, np_win, atol=1e-7, err_msg="Hann window implementations differ!"
    )
...
def test_fft_equivalence(rng_image: np.ndarray) -> None:
    ...
    np.testing.assert_allclose(
        scipy_mag, np_mag, rtol=1e-5, err_msg="FFT magnitude implementations differ!"
    )
...
def test_apply_fft_parity_on_nonzero(rng_image: np.ndarray) -> None:
    ...
    np.testing.assert_allclose(
        new, ref, rtol=1e-5, atol=1e-6, err_msg="apply_fft differs from original pipeline!"
    )
...
def test_log_zero_behavior() -> None:
    ...
    assert np.any(np.isneginf(new)), (
        "new_apply_fft should also produce -inf (parity with reference)"
    )
